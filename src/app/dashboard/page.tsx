            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className={getStatusColor(health?.invoice_api || "offline")}>{getStatusDot(health?.invoice_api || "offline")} {health?.invoice_api || "..."}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400">Contract API</span>
                  <span className={getStatusColor(health?.contract_api || "offline")}>{getStatusDot(health?.contract_api || "offline")} {health?.contract_api || "..."}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400">HydraulikDoc</span>
                  <span className={getStatusColor(health?.hydraulikdoc || "offline")}>{getStatusDot(health?.hydraulikdoc || "offline")} {health?.hydraulikdoc || "..."}</span>
                </div>
                <div className="flex justify-between text-sm">
